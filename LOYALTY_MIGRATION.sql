...
-- Project: luxecut (sdxfgugmdrmdjwhagjfa)
-- ============================================================================
...
-- Check if loyalty_settings table exists and has default row
SELECT * FROM loyalty_settings;

-- Check if loyalty_transactions table exists
SELECT COUNT(*) FROM loyalty_transactions;

-- Check if app_users has new columns
SELECT column_name, data_type 
FROM information_schema.columns 
WHERE table_name = 'app_users' 
AND column_name IN ('total_confirmed_visits', 'redeemable_points', 'status_tier');

-- Check RLS policies
SELECT schemaname, tablename, policyname 
FROM pg_policies 
WHERE tablename IN ('loyalty_settings', 'loyalty_transactions');
